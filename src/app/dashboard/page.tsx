  ...
  return (
    <div className="min-h-screen bg-[#060d1a]">
        ...
        {isLoading && !weather && (
          <div className="space-y-5 animate-pulse">
            ...
            <div className="grid grid-cols-4 gap-3">
              {[...Array(4)].map((_, i) => <div key={i} className="h-20 bg-slate-800/60 rounded-xl" />)}
            </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatCard
                icon="🌡️"
                label="Today's Range"
                value={today ? `${Math.round(today.temp_min)}° – ${Math.round(today.temp_max)}°C` : "--"}
                sub="Min / Max"
                color="orange"
              />
              <StatCard
                ...
                value={`${weeklyRain.toFixed(1)} mm`}
                sub={`${rainDays} rainy day${rainDays !== 1 ? "s" : ""}`}
                color="blue"
              />
              <StatCard
                icon="💨"
              ...
              <StatCard
                icon="🌅"
                label="Sunrise / Set"
                value={today ? `${fmtTime(today.sunrise)} / ${fmtTime(today.sunset)}` : "--"}
                sub="Today"
                color="yellow"
              />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              {risks ? (
                ...
                <div className="rounded-2xl bg-slate-800/60 backdrop-blur border border-slate-700/50 p-5 flex flex-col items-center justify-center min-h-[200px] gap-3">
                  ...
                  <p className="text-slate-400 text-sm font-medium">Select a crop to view risk analysis</p>
                  <p className="text-slate-600 text-xs text-center">Drought, flood, heat stress, frost and disease risks will appear here based on the 7-day forecast</p>
                </div>
              )}
            </div>
          </>
      ...
      <footer className="border-t border-slate-800/40 mt-10 py-5 text-center text-slate-600 text-xs">
        ...
        <a href="https://weather-ai.co" className="text-emerald-700 hover:text-emerald-500 transition-colors" target="_blank" rel="noopener noreferrer">
          WeatherAI
        </a>{" "}
...
function StatCard({ icon, label, value, sub, color }: {
  ...
  const colors = {
    orange: "from-orange-500/10 to-orange-500/5 border-orange-700/30",
    blue: "from-blue-500/10 to-blue-500/5 border-blue-700/30",
    purple: "from-purple-500/10 to-purple-500/5 border-purple-700/30",
    yellow: "from-yellow-500/10 to-yellow-500/5 border-yellow-700/30",
  };
  return (
    <div className={`rounded-xl bg-gradient-to-br ${colors[color]} border p-4 backdrop-blur`}>
